...
const GH_LOGO_URL  = 'https://github.com/darkthila5-arch/image/blob/main/logo.jpg?raw=true';
...
const runImageEffect = async (inputBuf, filterStr, outputExt = 'jpg', extraFfmpegArgs = []) => {
  ...
  try {
    ...
    await execFileAsync(ffmpegPath, ffArgs, { timeout: 90000 });
  ...
  try {
    switch (cmd) {
      ...
      case 'menu': {
        ...
        await sock.sendMessage(jid, {
          image: { url: 'https://files.catbox.moe/hz8vij.png' },
          caption: menuText,
            ...
            contextInfo: {
              externalAdReply: {
                ...
                ...(getDefaultLogoBuffer()
                  ...
                  : { thumbnailUrl: 'https://files.catbox.moe/hz8vij.png' }),
...
🔊 *Default:* nova`);
      ...
      case 'ytdl': {
        ...
        try {
          const ytdlInfoArgs = ['--no-playlist', '--print', '%(title)s|||%(duration)s|||%(filesize_approx)s', '--skip-download', '--quiet', ...YT_BYPASS_ARGS, args];
          const ytdlInfo = await execFileAsync(YT_DLP_BIN, ytdlInfoArgs, { timeout: 30000 });
          const [ytTitle, ytDur, ytSize] = ytdlInfo.stdout.trim().split('|||');
          const ytDurNum = parseInt(ytDur) || 0;
          if (ytDurNum > 600) {
          ...
          const ytdlDlArgs = [
            ...
            ...YT_BYPASS_ARGS,
            '-o', `${ytdlBase}.%(ext)s`, args,
          ];
          await execFileAsync(YT_DLP_BIN, ytdlDlArgs, { timeout: 120000 });
          let ytFile = `${ytdlBase}.mp4`;
          ...
          try { fs.unlinkSync(ytFile); } catch (_) {}
          const ytDurFmt = `${Math.floor(ytDurNum / 60)}:${String(ytDurNum % 60).padStart(2, '0')}`;
          await sock.sendMessage(jid, {
            video: ytBuf, mimetype: 'video/mp4',
            caption: `╭─「 🎬 ʏᴏᴜᴛᴜʙᴇ ᴠɪᴅᴇᴏ 」\n│ 🎬 *${ytTitle || 'YouTube Video'}*\n│ ⏱ ${ytDurFmt}\n╰──────────●●►\n\n> ${footer}`,
            fileName: `${(ytTitle || 'video').replace(/[^a-z0-9]/gi, '_')}.mp4`,
          }, { quoted: msg });
          await react(sock, msg, '✅');
        } catch (ytdlErr) {
